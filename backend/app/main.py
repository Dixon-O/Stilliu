...
async def lifespan(app: FastAPI):
    ...
    logger.info(
        "Stilliu backend starting. demo_mode=%s model=%s",
        settings.demo_mode,
...
async def analyze(req: AnalyzeRequest):
    ...
    try:
        async def _run_analysis():
            ...
            draft_vec = await loop.run_in_executor(None, mean_embedding, draft_paras)

            # 2. Generate + embed the bland baseline (concurrent with voice)
            baseline_text = await loop.run_in_executor(None, generate_baseline, req.draft)
            ...
            for d in raw_directions:
                ...
                direction_cards.append(DirectionCard(
                    persona=d["name"],
        ...
        return await asyncio.wait_for(_run_analysis(), timeout=settings.watsonx_timeout * 6)
